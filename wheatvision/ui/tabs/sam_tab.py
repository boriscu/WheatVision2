"""SAM segmentation tab for WheatVision2 UI."""
...
class SAMTab:
    """SAM segmentation tab component."""
    ...
    def build(self) -> None:
        """Build the SAM segmentation tab UI."""
        with gr.Row():
    ...
    def _run_pipeline(
        ...
        try:
            ...
            if self._state.sam_pipeline is None:
                self._state.sam_pipeline = SegmentationPipeline(
                    SegmentationModel.SAM,
                    enable_preprocessing=enable_preprocess,
                    enable_postprocessing=enable_postprocess,
                )

            self._state.sam_pipeline.set_preprocessing_enabled(enable_preprocess)
            self._state.sam_pipeline.set_postprocessing_enabled(enable_postprocess)

            if enable_preprocess:
                self._state.sam_pipeline.update_preprocessing_settings(
                    hsv_low=(h_low, s_low, v_low),
                    hsv_high=(h_high, s_high, v_high),
                )

            if enable_postprocess:
                self._state.sam_pipeline.update_postprocessing_settings(
                    min_aspect=min_aspect,
                    max_aspect=max_aspect,
                )

            frames, preprocess_results, seg_results, metrics = self._state.sam_pipeline.process_video(
                video_path, max_frames=int(max_frames)
            )

            self._state.sam_results = (frames, preprocess_results, seg_results, metrics)
    ...
    def _export_masks(self) -> Optional[str]:
        """Export SAM masks as ZIP."""
        if self._state.sam_results is None:
            return None
        _, _, results, _ = self._state.sam_results
        return self._export_masks_zip(results, "sam")
    ...
    def _export_video(self) -> Optional[str]:
        ...
        if self._state.sam_results is None:
            return None
        frames, _, results, _ = self._state.sam_results
    ...
    def _export_metrics(self) -> Optional[str]:
        ...
        if self._state.sam_results is None:
            return None
        _, _, _, metrics = self._state.sam_results
